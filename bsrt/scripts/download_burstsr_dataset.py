...
def download_train_folders(out_dir: str, i: int) -> None:
    if not os.path.isfile("{}/train_{:02d}.zip".format(out_dir, i)):
        print("Downloading train_{:02d}".format(i))

        urllib.request.urlretrieve(
            "https://data.vision.ee.ethz.ch/bhatg/BurstSRChallenge/train_{:02d}.zip".format(
                i
            ),
            "{}/tmp_{:02d}.zip".format(out_dir, i),
        )

        os.rename(
            "{}/tmp_{:02d}.zip".format(out_dir, i),
            "{}/train_{:02d}.zip".format(out_dir, i),
        )
