...
def main():
    train_zurich_raw2rgb = ZurichRAW2RGB(
        root="/data/dataset/ntire21/burstsr/synthetic", split="train"
    )
    train_data = SyntheticBurstTrainDataset(
        train_zurich_raw2rgb, burst_size=14, crop_sz=384
    )
    means = []
    stds = []
