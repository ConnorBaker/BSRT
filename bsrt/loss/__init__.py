...
class Loss(nn.modules.loss._Loss):
    def __init__(self, config: Config, ckp):
        ...
        for loss in config.loss.split("+"):
            ...
            if loss_type == "MSE":
                ...
                loss_function = getattr(module, "VGG")(
                    loss_type[3:], rgb_range=config.rgb_range
                )
            elif loss_type.find("GAN") >= 0:
                module = import_module("loss.adversarial")
                loss_function = getattr(module, "Adversarial")(config, loss_type)
        ...
        if not config.cpu and config.n_GPUs > 1:
            self.loss_module = nn.DataParallel(self.loss_module, range(config.n_GPUs))

        if config.load != "":
            self.load(ckp.dir, cpu=config.cpu)
