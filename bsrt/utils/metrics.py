...
class PSNR(nn.Module):
    ...
    def forward(self, pred, gt, valid=None):
        ...
        if valid is None:
            all_scores = [
                self.psnr(p.unsqueeze(0), g.unsqueeze(0)) for p, g in zip(pred, gt)
            ]
        else:
            all_scores = [
...
class AlignedL1(nn.Module):
    ...
    def forward(self, pred, gt, burst_input):
        ...
        with torch.no_grad():
            flow = self.alignment_net(
                pred / (pred.max() + 1e-6), gt / (gt.max() + 1e-6)
            )
        ...
        pred_warped_m, valid = match_colors(
            frame_gt_ds, burst_0_warped, pred_warped, self.ksz, self.gauss_kernel
        )
...
class AlignedL2(nn.Module):
    ...
    def forward(self, pred, gt, burst_input):
        ...
        # Match the colorspace between the prediction and ground truth
        pred_warped_m, valid = sca_utils.match_colors(
            frame_gt_ds, burst_0_warped, pred_warped, self.ksz, self.gauss_kernel
        )
...
class AlignedPSNR(nn.Module):
    ...
    def forward(self, pred, gt, burst_input):
        all_scores = [
            self.psnr(p.unsqueeze(0), g.unsqueeze(0), bi.unsqueeze(0))
