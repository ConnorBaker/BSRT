{
  ...
  ippicv = {
    ...
    in
      ...
      then {${name "lnx_intel64"} = "c0bd78adb4156bbf552c1dfe90599607";}
      else if stdenv.hostPlatform.system == "i686-linux"
      then {${name "lnx_ia32"} = "4f38432c30bfd6423164b7a24bbc98a0";}
      else if stdenv.hostPlatform.system == "x86_64-darwin"
      then {${name "mac_intel64"} = "fe6b2bb75ae0e3f19ad3ae1a31dfa4a2";}
...
in
  stdenv.mkDerivation {
    ...
    # TODO: Really? The whole toolkit?
    propagatedBuildInputs =
      lib.optional enablePython pythonPackages.numpy
      ++ lib.optionals enableCuda [cudaPackages.cudatoolkit cudaPackages.cudnn nvidia-optical-flow-sdk];
    ...
    cmakeFlags =
      [
        ...
        (opencvFlag "CUDNN" enableCuda)
        (opencvFlag "TBB" enableTbb)
      ]
      ++ lib.optionals enableCuda [
        "-DCUDA_FAST_MATH=ON"
        # We need to set the C, C++, and host compilers for CUDA to the same compiler.
        ...
        # We tell CMake the architecture of the GPU we're building for.
        "-DCUDA_ARCH_BIN=${
          lib.concatStringsSep ";" cudaPackages.cudaFlags.cudaRealCapabilities
        }"
